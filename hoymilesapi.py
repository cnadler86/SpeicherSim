...
from const import (BASE_URL, COOKIE_EGG_SESS, COOKIE_UID, GET_ALL_DEVICE_API,
                   ...
                   PAYLOAD_T2, PAYLOAD_T3, STATION_FIND, PAYLOAD_DETAILS, DATA_FIND_DETAILS)
...
class Hoymiles(object):
    ...
    def send_request(self, url: str, header: dict, payload: str, rtype: str):
        ...
        try:
            response = sess.send(prepped, **settings)
            ret = ""
    ...
    def request_history(self,startTime,endTime) -> dict:
        """Send request for solar data history

        Returns:
            _type_: _description_
        """
        template = Template(PAYLOAD_T3)
        payload = template.substitute(sid=self.plant_id,startTime=startTime,endTime=endTime)
